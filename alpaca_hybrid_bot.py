#!/usr/bin/env python3
# GROK_OKX_APEX_V8 - HYBRID STRATEGY (WORKING VERSION)
...
# ==============================================================================
# PROMETHEUS METRICS (WORKING VERSION - PORT 3000)
# ==============================================================================
...
class GrokApexIroncladBot:
    def __init__(self, paper_mode: bool = True, interval_minutes: int = 5):
        ...
        # START METRICS ON PORT 3000 (WORKING CONFIGURATION)
            ...
            for symbol, price, df in results:
                ...
                try:
                    ...
                    if score > self.buy_threshold and symbol not in self.positions:
                        ...
                        self.positions[symbol] = {'price': price, 'entry_time': datetime.now().isoformat()}
                        HYBRID_POSITION_STATUS.labels(symbol=symbol).set(1)
                        self.save_state()
                        ...
                        logger.info(f"SELL: {symbol} @ ${price:.2f} (PnL: {pnl_pct:.2f}%)")
